...
fit2 <- sem(model, data=dat, se='bootstrap', bootstrap=10000)
...
indirect2:= r.i*i.c"
fit1 <- sem(model, data=dat)
fit2 <- sem(model, data=dat, se='bootstrap')
fit3 <- sem(model, data=dat, mimic='Mplus')
...
```{r}
fit2 <- sem(model, data=dat, se='bootstrap', bootstrap=10000, std.ov = TRUE,
...
model <- "
Recall ~ X2 + X3 + X4 + X5"
fit <- sem(model, dat, se='bootstrap')
parameterEstimates(fit)
```
